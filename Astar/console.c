...
static void con_setCursor(int show) {  
    CONSOLE_CURSOR_INFO ci = {CONSOLE_CURSOR_SIZE, show}; //заполненость и прозрачность курсора
    HANDLE hStdOut = GetStdHandle(STD_OUTPUT_HANDLE); //устройство вывода (опусти перо)
...
int con_getMaxXY(int *px, int *py) //записываем информацию о размерах
{
    CONSOLE_SCREEN_BUFFER_INFO csbi;
...
int con_clearScr()
{
    ...
    if (con_getMaxXY(&mx, &my))	//длина ширина ( max X, max Y)
    {
        COORD szBuf = {mx + 1, my + 1};
        COORD BufferCoord= {0, 0};
        SMALL_RECT srWriteRegion = {0, 0, mx, my}; //small rect содержит инфу о крайних точках
        int nResult;
